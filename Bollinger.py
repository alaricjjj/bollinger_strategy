    ...
    def next(self):
        if len(self) > self.period_long:
            ...
            if self.position.size > 0:
                if self.order_buy_stop == None and self.order_buy_trail == None:
                    self.cancel_all_order()
                    self.order_buy_stop = self.sell(size= abs(self.position.size),
                                                    price= self.position.price / self.back_stop_percentage_long,
                                                    exectype=bt.Order.Stop )
                    self.order_buy_trail = self.sell(size=abs(self.position.size),
                                                     exectype=bt.Order.StopTrail,
                                                     trailpercent=self.trail_stop_percentage_long,
                                                     oco=self.order_buy_stop)
                if self.order_buy_stop.status not in [1, 2] and self.order_buy_trail.status not in [1, 2]:
                    self.cancel_all_order()
                    self.order_buy_stop = self.sell(size=abs(self.position.size),
                                                    price=self.position.price / self.back_stop_percentage_long,
                                                    exectype=bt.Order.Stop)
                    self.order_buy_trail =self.sell(size=abs(self.position.size), exectype=bt.Order.StopTrail,
                                            trailpercent=self.trail_stop_percentage_long, oco=self.order_buy_stop)


            if self.position.size < 0:
                if self.order_sell_stop  == None and self.order_sell_trail == None:
                    self.cancel_all_order()
                    self.order_sell_stop = self.buy(size= abs(self.position.size),
                                                    price= self.position.price * self.back_stop_percentage_long,
                                                    exectype=bt.Order.Stop )
                    self.order_sell_trail = self.buy(size=abs(self.position.size),
                                                     exectype=bt.Order.StopTrail,
                                                     trailpercent=self.trail_stop_percentage_long, oco=self.order_sell_stop)
                if self.order_sell_stop.status not in [1, 2] and self.order_sell_trail.status not in [1, 2]:
                    self.cancel_all_order()
                    self.order_sell_stop = self.buy(size=abs(self.position.size),
                                                    price=self.position.price * self.back_stop_percentage_long,
                                                    exectype=bt.Order.Stop)
                    self.order_sell_trail = self.buy(size=abs(self.position.size), exectype=bt.Order.StopTrail,
                                                 trailpercent=self.trail_stop_percentage_long, oco=self.order_sell_stop)
